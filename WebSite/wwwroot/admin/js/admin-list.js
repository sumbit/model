...
var adminList = {
    ...
    bootStrapTable: function (options) {
        var defaults = {
            ...
            striped: true,
            method: "post",
            classes: "table table-bordered table-hover dataTable table-striped width-full text-nowrap",
            url: "",
            idField: "_ukid",
        ...
        var jsonConfig = {
            ...
            onClickRow: function (row, dom, field) {
                //console.log(options.idField);
                //console.log(row[options.idField]);
                //console.log(dom);
                //console.log(field);
                ...
                if (options.onClickRow != null) {
                    ...
                    adminList.bootStrapTableIndex = row[options.idField];
            ...
            onLoadSuccess: function () {
                ...
                if (adminList.bootStrapTableIndex) {
                    adminList.bootStrapTableObj.bootstrapTable("checkBy", { field: options.idField, values: [adminList.bootStrapTableIndex] });
                }
        ...
        $(window).bind('resize', function () {
            adminList.bootStrapTableObj.bootstrapTable('resetView', { height: $(window).height() - 80 });
        });
    ...
    //删除数据
    delete: function (url, fieldId, callBack) {
        var rows = adminList.selectRows();
        if (rows.length == 0) {
        ...
        admin.confirm("确认删除?", function (index) {
            ...
            for (var i = 0; i < rows.length; i++) {
                //console.log(rows[i][fieldId]);
                json.push(rows[i][fieldId]);
            }
            admin.ajax({
                url: url,
